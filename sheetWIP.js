...
updateSimilarities(queriesData, queriesDataClone, 'activeSheet', 'selectedColumnIndex', 'columnIndex'); // 'activeSheet', 'selectedColumnIndex', 'columnIndex' are for Google Sheet)
...
function calculateSimilarityScore(urls1, urls2, commonURLs) {
    ...
    const maxScore1 = urls1.reduce((sum, _, i) => sum + (i + 1) ** 2, 0);
    const maxScore2 = urls2.reduce((sum, _, i) => sum + (i + 1) ** 2, 0);
    const maxScore = Math.min(maxScore1, maxScore2);

    const similarityPercentage = score / maxScore * 100;

    return { score, similarityPercentage };
}
...
function updateSimilarities(queriesData, queriesDataClone, sheet, selectedColumnIndex, cannibalisationColumnIndex) {
    for (const queryData of queriesData) {
        ...
        for (const queryDataClone of queriesDataClone) {
            if (queryData.Query !== queryDataClone.Query) {
                ...
                if (commonURLs.length > 0) {
                    const { score, similarityPercentage } = calculateSimilarityScore(queryData.URL, queryDataClone.URL, commonURLs);
                    ...
                    similarityList.push(queryDataClone.Query + ' ' + similarityPercentage);
                }
            }
        }
        ...
        if (similarityList.length > 0) {
            ...
            // sheet.getRange(rowIndex, cannibalisationColumnIndex).setValue(similarityList.join(', '));// for Google Sheet
        }
    }
}
